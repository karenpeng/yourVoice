...
var p5SOUND = (function () {
  ...
  if (typeof window.AudioBufferSourceNode.prototype.start !== 'function') {
    window.AudioBufferSourceNode.prototype.start = window.AudioBufferSourceNode
      .prototype.noteGrainOn;
  }
  if (typeof window.AudioBufferSourceNode.prototype.stop !== 'function') {
    window.AudioBufferSourceNode.prototype.stop = window.AudioBufferSourceNode
      .prototype.noteOff;
  }
  if (typeof window.OscillatorNode.prototype.start !== 'function') {
    window.OscillatorNode.prototype.start = window.OscillatorNode.prototype
      .noteOn;
  }
  ...
  if (!window.AudioContext.prototype.hasOwnProperty('createScriptProcessor')) {
    window.AudioContext.prototype.createScriptProcessor = window.AudioContext
      .prototype.createJavaScriptNode;
  }
  ...
  p5.prototype.SoundFile.prototype.currentTime = function () {
    ...
    if (this.isPlaying() && !this.unpaused) {
      howLong = ((this.p5s.audiocontext.currentTime - this.startSeconds +
        this.startTime) * this.source.playbackRate.value) % this.duration();
      return howLong;
    } else if (this.isPlaying() && this.unpaused) {
      howLong = (this.pauseTime + (this.p5s.audiocontext.currentTime - this
        .startSeconds) * this.source.playbackRate.value) % this.duration();
      return howLong;
    } else if (this.paused) {
  ...
  p5.prototype.Amplitude.prototype.setInput = function (snd, smoothing) {
    ...
    // But for now, it just connects to master.
      ...
      console.log(
        'source is not ready to connect. Connecting to master output instead'
      );
  ...
  p5.prototype.Amplitude.prototype.volumeAudioProcess = function (event) {
    ...
    for (var i = 0; i < bufLength; i++) {
      ...
      if (this.normalize) {
        ...
        sum += constrain(x / this.volMax, -1, 1) * constrain(x / this.volMax, -
          1, 1);
      } else {
        total += x;
        sum += x * x;
   ...
   */
  p5.prototype.FFT = function (smoothing, fft_size, minDecibels,
    maxDecibels) {
  ...
  p5.prototype.Oscillator.prototype.start = function (time) {
    if (!this.started) {
      ...
      this.oscillator = this.p5s.audiocontext.createOscillator();
      this.oscillator.frequency.exponentialRampToValueAtTime(abs(freq),
        this.p5s.audiocontext.currentTime);
  ...
  p5.prototype.Oscillator.prototype.freq = function (val, t) {
    ...
    this.oscillator.frequency.exponentialRampToValueAtTime(val, rampTime +
      this.p5s.audiocontext.currentTime);
  };
  ...
  p5.prototype.Pulse = function (freq, w) {
    ...
    this.dNode.delayTime.value = map(this.w, 0, 1.0, 0, 1 / this.oscillator
      .frequency.value);
  ...
  p5.prototype.Pulse.prototype.start = function (time) {
    if (!this.started) {
      ...
      // var detune = this.oscillator.frequency.value;
      this.oscillator = this.p5s.audiocontext.createOscillator();
      this.oscillator.frequency.exponentialRampToValueAtTime(freq, this.p5s
        .audiocontext.currentTime);
      this.oscillator.type = type;
      // this.oscillator.detune.value = detune;
      this.oscillator.connect(this.output);
      this.started = true;
      time = time + this.p5s.audiocontext.currentTime || this.p5s.audiocontext
        .currentTime;
      this.oscillator.start(time);
      ...
      this.osc2.oscillator.frequency.exponentialRampToValueAtTime(freq,
        this.p5s.audiocontext.currentTime);
  ...
  p5.prototype.Pulse.prototype.freq = function (val, t) {
    var rampTime = t || 0;
    this.oscillator.frequency.cancelScheduledValues(this.p5s.audiocontext.currentTime);
    this.osc2.oscillator.frequency.cancelScheduledValues(this.p5s.audiocontext
      .currentTime);
    this.oscillator.frequency.exponentialRampToValueAtTime(val, rampTime +
      this.p5s.audiocontext.currentTime);
    this.osc2.oscillator.frequency.exponentialRampToValueAtTime(val,
      rampTime + this.p5s.audiocontext.currentTime);
  };
    ...
    if (p5sound.inputSources[self.currentSource]) {
      ...
      navigator.getUserMedia(constraints,
        this._onStream = function (stream) {
          ...
          self.mediaStream = self.p5s.audiocontext.createMediaStreamSource(
            stream);
